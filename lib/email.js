...
export async function sendEventAvailabilityAlertEmail(to, eventName, eventUrl, lowestAsk) {
  const hasPrice = Number.isFinite(Number(lowestAsk)) && Number(lowestAsk) > 0;
  const priceText = hasPrice
    ? `Laagste prijs op dit moment: &euro; ${formatPrice(Number(lowestAsk))} (excl. servicekosten).`
    : 'Er is nieuw aanbod beschikbaar voor dit event.';

  const html = layout(`
    <h1 style="margin:0 0 8px;font-size:22px;color:#0f172a;">Er is een ticket beschikbaar gekomen 🎉</h1>
    <p style="margin:0 0 16px;font-size:14px;color:#475569;line-height:1.6;">
      Goed nieuws: er is weer aanbod voor <strong>${eventName}</strong> op Tckr.
    </p>
    <p style="margin:0 0 16px;font-size:14px;color:#475569;line-height:1.6;">
      ${priceText}
    </p>
    ${button(eventUrl, 'Bekijk het aanbod')}
    <p style="margin:0;font-size:13px;color:#94a3b8;line-height:1.6;">
      We hebben je melding geactiveerd toen er nog geen tickets beschikbaar waren.
    </p>
  `);
  ...
  const { data, error } = await resend.emails.send({
    ...
    to,
    replyTo: REPLY_TO,
    subject: `Ticket beschikbaar: ${eventName} | Tckr`,
    html,
  });

  if (error) {
    console.error('[Email] Failed to send event availability alert email:', error);
  }
  return { data, error };
}
